...
class DilatedResnetEncoder(nn.Module):
    ...
    def __init__(self, model, in_channels=3,dilate_scale=8):
        ...
        else: raise KeyError("Dilation supported only for resnet34,resnet50")
        
        if in_channels !=3:
                weight=model.conv1.weight
                out_channels=model.conv1.out_channels
                model.conv1=torch.nn.Conv2d(in_channels,out_channels,kernel_size=model.conv1.kernel_size, stride=model.conv1.stride, padding=model.conv1.padding, bias=False).cuda()
                model.conv1.weight[:,:3].data= torch.nn.Parameter(weight.data).cuda()
                for i in range(3,in_channels): model.conv1.weight[:,i].data=torch.nn.Parameter(weight[:,0].data).cuda()
        ...
        self.maxpool = orig_resnet.maxpool; self.maxpool.return_indices=True # **
...
class ResNetEncoder(ResNet, EncoderMixin):
    ...
    def forward(self, x):
        ...
        for i in range(self._depth + 1):
            x = stages[i](x)
            features.append(x)
        for i,e in enumerate(features): print(f'ResNetEncoderfeat_{i}',e.size())
        return features
